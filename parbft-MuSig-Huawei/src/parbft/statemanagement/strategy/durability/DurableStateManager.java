 ...
 */
package parbft.statemanagement.strategy.durability;
...
import parbft.statemanagement.ApplicationState;
import parbft.statemanagement.SMMessage;
import parbft.statemanagement.strategy.BaseStateManager;
import parbft.tom.core.DeliveryThread;
import parbft.tom.core.TOMLayer;
import parbft.tom.server.defaultservices.CommandsInfo;
import parbft.tom.server.defaultservices.durability.DurabilityCoordinator;
...
public class DurableStateManager extends BaseStateManager {
	...
	protected void requestState() {
		...
		int[] otherProcesses = SVController.getCurrentViewOtherBackups(SVController.getStaticConf().getProcessId());
		...
		tomLayer.getCommunication().send(
				SVController.getCurrentViewOtherBackups(SVController.getStaticConf().getProcessId()), cstMsg);
...
//						}
						...
						if (appStateOnly) {
							...
							&& (SVController.getCurrentViewN(SVController.getStaticConf().getGroup(SVController.getStaticConf().getProcessId())) / 2) < getReplies()) {
						logger.warn("---- DIDNT RECEIVE STATE ----");

						logger.debug("I have more than "
								+ (SVController.getCurrentViewN(SVController.getStaticConf().getGroup(SVController.getStaticConf().getProcessId())) / 2)
