...
import parbft.communication.client.ReplyListener;
...
public class AsynchServiceProxy extends ServiceProxy {
     ...
     * @see// bellow
     */
    public AsynchServiceProxy(int processId,int group) {
        this(processId,group, null);
        init();
    }
     ...
     * @see //bellow
     */
    public AsynchServiceProxy(int processId,int group, String configHome) {
        super(processId,group, configHome);
        toGroup = group;
        init();
    }
     ...
     * @see// bellow
     */
    public AsynchServiceProxy(int processId,int group, String configHome, KeyLoader loader) {
        super(processId,group, configHome, loader);
        init();
    }
    ...
    public AsynchServiceProxy(int processId,int group, String configHome,
        ...
        super(processId,group, configHome, replyComparator, replyExtractor, loader);
        init();
    }
     ...
     * @see bellow
     */
    public int invokeAsynchRequest(byte[] request, ReplyListener replyListener, TOMMessageType reqType,int group) {
        this.toGroup = group;
        /////////////////////////////////////////////////////////////////////////////
        return invokeAsynchRequest(request, super.getViewManager().getCurrentViewProcesses(toGroup), replyListener, reqType);
        /////////////////////////////////////////////////////////////////////////////
    }
                ...
                if (replyListener != null) {
                    ...
                    if ((v = newView(reply.getContent())) != null && !requestsAlias.containsKey(reply.getOperationId())) { // Deal with a system reconfiguration
                        ...
                        for (int i = 0; i < replies.length; i++) {
                            ...
                            if ((replies[i] != null) && (i != pos || getViewManager().getCurrentViewN(toGroup) == 1)
    ...
    private int invokeAsynch(byte[] request, int[] targets, ReplyListener replyListener, TOMMessageType reqType) {
        ...
        try {
            ...
            requestsReplies.put(requestContext.getOperationId(), new TOMMessage[super.getViewManager().getCurrentViewN(toGroup)]);
/****************************************7.21********************************************problemhere***************************/
            sendMessageToTargets(request, requestContext.getReqId(), requestContext.getOperationId(), targets, reqType,0);
/***************************************************************************************************************/
        } finally {
            canSendLock.unlock();
        }
