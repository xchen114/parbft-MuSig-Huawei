...
import parbft.tom.core.TOMSender;
...
import parbft.reconfiguration.ReconfigureReply;
...
import parbft.tom.util.Extractor;
...
public class ServiceProxy extends TOMSender {
	 ...
	 * see bellow
	 */
	public ServiceProxy(int processId,int group) {
		this(processId, group,null, null, null, null);
	}
	 ...
	 * see bellow
	 */
	public ServiceProxy(int processId,int group, String configHome) {
		this(processId,group, configHome, null, null, null);
	}
	 ...
	 * see bellow
	 */
	public ServiceProxy(int processId,int group, String configHome, KeyLoader loader) {
		this(processId,group, configHome, null, null, loader);
	}
	...
	public ServiceProxy(int processId,int group, String configHome,
		...
		if (configHome == null) {
			init(processId,group, loader);
		} else {
			init(processId,group, configHome, loader);
		}
		toGroup = group;

		replies = new TOMMessage[getViewManager().getCurrentViewN(toGroup)];
	...
	public byte[] invoke(byte[] request, TOMMessageType reqType) {
		...
		if(requestType == TOMMessageType.UNORDERED_HASHED_REQUEST){
			...
			TOMulticast(sm,toGroup);
		}else{
			TOMulticast(request, reqId, operationId, reqType,toGroup);
		}
...
/************************************************************************************/
				if(requestType == TOMMessageType.UNORDERED_HASHED_REQUEST)
				{
					...
					for (int i = 0; i < replies.length; i++) {
						...
						if ((i != pos || getViewManager().getCurrentViewN(toGroup) == 1) && replies[i] != null
				...
				if (response == null) {
					if (requestType.equals(TOMMessageType.ORDERED_REQUEST)) {
						if (receivedReplies == getViewManager().getCurrentViewN(toGroup)) {
							reqId = -1;
							this.sm.release(); // resumes the thread that is executing the "invoke" method
						}
					}else if (requestType.equals(TOMMessageType.UNORDERED_HASHED_REQUEST)) {
						if (hashResponseController.getNumberReplies() == getViewManager().getCurrentViewN(toGroup)) {
							reqId = -1;
							this.sm.release(); // resumes the thread that is executing the "invoke" method
						}
					} else {  // UNORDERED
	...
	protected int getReplyQuorum() {
		if (getViewManager().getStaticConf().isBFT()) {
			return (int) Math.ceil((getViewManager().getCurrentViewN(toGroup)
					+ getViewManager().getCurrentViewF()) / 2) + 1;
		} else {
			return (int) Math.ceil((getViewManager().getCurrentViewN(toGroup)) / 2) + 1;
		}
	}
	...
	private class HashResponseController{
		...
		public TOMMessage getResponse(int pos, TOMMessage tomMessage){
			...
			if(hashReplies[replyServerPos]!=null){
				int sameContent = 1;
				for (int i = 0; i < replies.length; i++) {
					if ((i != replyServerPos || getViewManager().getCurrentViewN(toGroup) == 1) && hashReplies[i] != null
