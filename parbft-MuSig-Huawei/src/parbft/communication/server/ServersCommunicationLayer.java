...
*/
package parbft.communication.server;
...
public class ServersCommunicationLayer extends Thread {
    ...
    private HashMap<Integer, ServerConnection> leaderConnections = new HashMap<>();
    ...
    public ServersCommunicationLayer(ServerViewController controller,
        ...
        this.group = controller.getStaticConf().getGroup(me);
        ...
        if (controller.isInCurrentView()) {
            int[] initialV = controller.getCurrentViewBackups(group);
            logger.info("Group:"+group);
            for (int i = 0; i < initialV.length; i++) {

                if (initialV[i] != me&&group == controller.getStaticConf().getGroup(initialV[i])) {
                    getConnection(initialV[i]);
                }
            }
            if(controller.isInitLeader()){
                int[] initialLeader = controller.getCurrentViewLeader();
                logger.debug("I am init leader~");
                for(int i : initialLeader){
                    if(i!=me){
                        getLeaderConnection(i);
                    }
                }
...
//        serverSocket.setSoTimeout(10000);
    ...
    public void updateConnections() {
        ...
        if (this.controller.isInCurrentView()) {
            ...
            int[] newV = controller.getCurrentViewBackups(group);
            for (int i = 0; i < newV.length; i++) {
                if (newV[i] != me&&group == controller.getStaticConf().getGroup(newV[i])) {
    ...
    //******* EDUARDO END **************//
    /************************************7.21***************************************/
    private ServerConnection getLeaderConnection(int remoteId) {
        connectionsLock.lock();
        ServerConnection ret = this.leaderConnections.get(remoteId);
        if (ret == null) {
            ret = new ServerConnection(controller, null, remoteId, this.inQueue, this.replica);
            this.leaderConnections.put(remoteId, ret);
        }
        connectionsLock.unlock();
        return ret;
    }
    public final void sendingToOtherLeader(int target, SystemMessage sm, boolean useMAC) {
        ...
        try {
            getLeaderConnection(target).send(data, useMAC);
...
//                    }
    ...
    public void shutdown() {
        ...
        //******* EDUARDO BEGIN **************//
        int[] activeServers = controller.getCurrentViewBackups(group);
    ...
    private void establishConnection(Socket newSocket, int remoteId) throws IOException {
        if(controller.isInitLeader(remoteId)&&controller.getStaticConf().getGroup(remoteId)!=controller.getStaticConf().getGroup(me)){
            this.leaderConnections.put(remoteId, new ServerConnection(controller, newSocket, remoteId, inQueue, replica));
        }
        else if ((this.controller.getStaticConf().getTTPId() == remoteId) || this.controller.isCurrentViewMember(remoteId)) {
            connectionsLock.lock();
